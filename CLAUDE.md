...
- **Eight apps as launchers**: `run_data_explorer()` (full pipeline: About → Import → Reshape → Summarize → Visualize → Map → Compare Groups → Regression → Export & Report), `run_reshape_tool()` (import → reshape → export & report), `run_combine_tool()` (import two tables → combine → export & report), `run_compare_groups()` (import → compare → export & report), `run_lmer_tool()` (import → mixed model → export & report), `run_glmm_review()` (import → general GLMM / binary 0-1 GLMM → export & report; contributed by a collaborator and absorbed to the house pattern in 0.7.0), `run_map_tool()` (import → interactive leaflet map → export & report), `run_regression_tool()` (import → regression → export & report). **As of 0.9.0 every app carries one "Export & Report" tab** (`exportReportUI()` composes the two modules into sub-tabs); the report's contents vary per app because the apps produce different things. Each has a `*_app()` builder returning the `shinyApp` object.
- **Twelve modules**: `mod_import` (upload/clean/recast + **column rename** (rename_column; active row filters follow the new name) + Data Health incl. **outlier flagging** + **row filter** + profile + **session save/restore**), `mod_reshape` (stack/split/transpose/sort/subset/**summary**), `mod_summarize`, `mod_visualize` (1–8 plots (MAX_PLOTS), **11 chart types** incl. bubble, lazy render, code export, **per-value-chart x cap** — box/violin/mean-error (any x) and bar (numeric x) BLOCK past X_LEVELS_MAX=100 distinct x values via `plot_block_reason()` (builder refuses, plot pane explains, code panel emits a comment; dates exempt, discrete bar lumps instead), **x-axis tick-label control** — crowded discrete axes auto-thin to ~12 evenly spaced labels (30 flipped) via `x_break_spec`/`x_axis_scale`, per-plot override + a date-format picker rendered only for a real Date x), `mod_map` (**interactive leaflet maps, two types**: points — coordinate auto-detect + swap, 4 basemaps, color/size-by with **log/quantile scales for both** and a graduated **size legend**, popups/hover labels, auto-clustering, layer groups, optional **density heatmap** (leaflet.extras, Suggests-gated) and **scale bar**, pan/zoom-preserving rebuilds; or **choropleth** — a built-in boundary set (US states / counties / world countries) or uploaded GeoJSON, joined to a data column with loud match diagnostics, **region stats on hover/click** — `choro_region_info` aggregates + builds per-region popups (rows/mean/median/sd/min/max) in one pass, click popups ride leaflet's native feature.properties.popup binding, hover tooltips via one `CHORO_TOOLTIP_JS` onRender hook, on by default with a checkbox off-switch, mirrored into the code export; HTML/PNG download + leaflet code export — the map downloads live in the module, NOT mod_export, whose chart slot is ggplot-only; snapshots render at the on-screen pane size read from session$clientData at DPR 1, see helpers_map notes), `mod_compare` (t-test/ANOVA/Wilcoxon/Kruskal + chi-square; **grid testing of many outcomes x many groups** with a BH-corrected summary + per-combination accordion, optionally **split by a third variable** (one stratum per level, capped at 6, BH across the whole family); assumptions, effect sizes, Tukey/**Dunn**/**Steel-Dwass** post-hoc + connecting letters, table percentages; the categorical pickers are labelled **rows / columns** with a Swap button, and every matrix renderer heads its leading column with the row variable's name — `table()` is called with calls so the matrices themselves carry no dimnames), `mod_regression` (**linear + logistic** via a spec-driven engine: categorical predictors with reference levels, interactions, polynomial, coefficient table with CIs, fit stats, **EMMeans + cld** reusing the lmer emm helpers, Q-Q/scale-location/Cook's diagnostics + assumption panel + hand-rolled **VIF**, odds ratios, save-A/fit-B nested comparison, code export), `mod_export` (data + charts + summary + model; optional `preview` arg), `mod_combine` (concatenate/join/update/compare), `mod_report` (one-click HTML **or Word** report of the whole session, with a **per-section picker** — one checkbox per wired stage, deselection applied BEFORE `report_spec()` so a dropped section is never rasterized; also exports `exportReportUI()`, the Export+Report sub-tab composer), `mod_lmer` (**linear mixed models** via lmerTest/emmeans: ANOVA, fit/variance, residuals, **EMMeans + cld**, interaction test, model comparison), `mod_glmm` (**generalized linear mixed models** via glmmTMB/DHARMa/emmeans — instantiated TWICE per app: `glmmUI(id, binary = FALSE)` for the general tab (**9-family picker** incl. ordered beta for proportions on the closed [0,1], live domain notes + pre-fit range check, ziformula/dispformula side-models) and `binary = TRUE` for the 0/1 tab (link picker; **Response-format radio: individual 0/1 rows or grouped successes/trials counts** — failures computed as `cbind(successes, trials - successes)`, validated by `glmm_trials_check`; NO dispformula because glmmTMB ignores it for the binomial family (measured: identical AIC/npar), while grouped counts CAN be overdispersed so the notes name betabinomial/OLRE; `fit$bernoulli`, not `fit$binary`, decides whether testZeroInflation runs); Wald ANOVA (car-gated), DHARMa residual battery, response-scale EMMeans + cld reusing `lmer_emm_plot`; returns `list(data =, report =)` — the augmented dataset plus a model-report payload — and the launcher unions combos from both instances into the export frame and passes BOTH payloads to the report).
- **Sixteen helper files** (15 `R/helpers_*` + `components`), all unit-tested. `helpers_examples.R` is the **example-dataset registry**: `EXAMPLE_SETS` (key -> label/tags/lazy builder closure, the `MAP_BUILTIN_BOUNDARIES` shape), nine per-app menu constants (`EXAMPLES_EXPLORER` ...), `as_example_spec`/`example_menu`/`example_build` (one lookup feeds both `importUI`'s menu and `importServer`'s loader, so labels and data cannot desync; unknown keys stop at app build), the exported `foxplots_examples()`/`foxplots_example()`, and the two generators `make_timeseries_example_data` (1,095 rows, real Date + the same date as text — deliberately just over BIG_ROWS so Visualize goes static, where axis crowding is visible) and `make_messy_example_data` (fires all nine Data Health detectors). `helpers_report.R` renders the session into pandoc-free HTML/Word; the mixed-model tools feed it through **`model_payload()`** (title / formulas / named tables / verbatim texts / notes / code) so ONE renderer per format (`.section_mixed`, `.docx_section_mixed`) serves any model engine — `report_spec(mixed =)` takes a LIST of payloads, which is how the GLMM app reports its general and binary fits side by side. `helpers_glmm.R` holds the GLMM engine (family registry `GLMM_FAMILY_CHOICES`/`glmm_family`/`GLMM_FAMILY_NOTE` + advisory `glmm_domain_check`, `build_side_formula` (~0/~1/~vars), `glmm_formula_string` (no transform, random part optional — GLM fallback noted; `quote_response = FALSE` passes a pre-built `cbind()` LHS through un-backticked), grouped-binomial kit (`glmm_count_cols`/`glmm_trials_check` — same NULL-or-one-string contract as the domain check, so the live warning and the fit error are literally the same message — /`glmm_cbind_response`/`glmm_trials_guess`), `glmm_fit`/`glmm_fit_stats`/`glmm_anova` (car-gated)/`glmm_pearson_ratio`, DHARMa wrappers `glmm_dharma`/`glmm_dharma_tests` (plot = FALSE — a pure helper must not open a device)/`glmm_dharma_plot` (readable labels: signif-3 predicted-response boxplot labels, annotation-free QQ), `glmm_emmeans` (response scale, reuses the lmer emm kit), code generators, `make_glmm_example_data` (RNG-snapshotted)); validated in tests against direct glmmTMB calls. `helpers_lmer.R` holds the mixed-model engine (formula/spec builders, `lmer_fit`/`lmer_emmeans`/`lmer_anova`/`lmer_compare`/`lmer_fit_stats`/`lmer_cook`, `make_example_data`), validated against the source repo's `validate_engine.R` reference values. `helpers_map.R` holds the map engine (`detect_coord_cols`/`clean_coords`/`build_leaflet_map` incl. **layer groups**, **log/quantile color AND size scales**, the size-legend builders, the **heatmap/scale-bar** blocks, and the **choropleth path** (`parse_geojson`/`geojson_props`/`geojson_bounds`/`build_choropleth` — per-feature style written into feature.properties.style, no sf needed, match diagnostics in attr "choro_diag") /`generate_map_code` (+ `generate_choropleth_code`)/`map_palette*`/`make_map_example_data` + the exporters: `inline_html_deps` makes the HTML download **self-contained without pandoc** by data-URI-ing every local script/stylesheet/asset, and `save_map_png` retries once with a fresh browser in modern headless mode, with Edge-as-Chrome fallback); its palette tree mirrors `group_scales`/`palette_code` exactly.
- **~60 exported functions** (16 launchers + `uf_logo_uri` + the example-data kit — `make_example_data`, `make_map_example_data`, `make_glmm_example_data`, `make_timeseries_example_data`, `make_messy_example_data`, `foxplots_examples`, `foxplots_example` — + ~36 helpers); internal functions are `@noRd`. `helpers_model.R` holds the fixed-effects regression engine (`reg_spec`/`reg_validate`/`reg_fit` incl. glm-binomial, `reg_coef_table`/`reg_fit_stats`/`reg_odds_ratios`/`reg_emmeans`/`reg_assumptions`/`reg_vif`/`reg_compare`/`reg_code`; `fit_model()` stays as a thin exported wrapper).
...
- **A testthat suite** (`tests/testthat/`, ~2,400 expectations) run via `R CMD check` / `testthat::test_local()` (Word-report tests `skip_if_not_installed("officer")`; the mixed-model engine tests `skip_if_not_installed("lmerTest"/"emmeans")`). Since 0.10.0 the app layer is covered too: **`test-modules.R`** (one contract check per module) and **`test-launchers.R`** (each app builds, serves a real page, and has its whole server run via `testServer` on the app object, plus a structural check that every `<mod>Server("id")` has a matching `<mod>UI("id")` — a mistyped id otherwise runs in a namespace with no UI and the tab is silently blank). testServer **works on the macOS dev box** (the segfault was Windows-only); read a module's return with `session$returned()` — but modules that return a LIST (`mod_lmer`, `mod_glmm`) expose it as `session$returned` directly, not callable.
- **`TESTING_CHECKLIST.md`** is the manual counterpart: per-app things to click, edge cases, and what good looks like. New caps/limits belong in its table.
...
├── CLAUDE.md / BUILD_LOG.md / STUDY_GUIDE.md / HOW_TO_USE.md / README.md / NEWS.md / TESTING_CHECKLIST.md
...
│   ├── helpers_plot.R           # build_full_plot (11 chart types), chart hints, palettes, code gen, grid export; caps BAR_MAX/PIE_MAX/GROUP_MAX/AXIS_LABEL_MAX/X_LEVELS_MAX (plot_block_reason — the per-value-chart block) + x_break_spec/x_axis_scale/x_axis_code (tick-label thinning)
│   ├── helpers_examples.R       # example-dataset registry (EXAMPLE_SETS + per-app menus + foxplots_example[s]) + make_timeseries_/make_messy_example_data
│   ├── helpers_model.R          # fit_model, model_interpretation, diagnostic ggplots
│   ├── helpers_reshape.R        # do_stack/do_split/do_transpose/do_sort/do_subset + reshape_hint (cross-feature advisory)
...
│   ├── helpers_report.R         # report_spec (+ model_payload for the mixed-model tools) + build_report_html / build_report_docx (pandoc-free HTML + editable Word) + render_report
...
│   ├── helpers_glmm.R           # GLMM engine: family registry + domain check, side formulas, glmm_fit/_anova/_dharma*/_emmeans, make_glmm_example_data
│   ├── helpers_map.R            # map engine: coord detect/clean, build_leaflet_map, generate_map_code, palettes, built-in boundaries (MAP_BUILTIN_BOUNDARIES + builtin_boundary_*), geojson_prop_examples/_choices, map_no_coord_advice, make_map_example_data, HTML/PNG export
│   ├── mod_import.R … mod_report.R   # the twelve Shiny modules (<feature>UI/<feature>Server), incl. mod_lmer + mod_glmm + mod_map
...
│   ├── run_glmm_review.R        # glmm_review_app()   + run_glmm_review()
│   ├── run_map_tool.R           # map_tool_app()      + run_map_tool()
│   └── run_regression_tool.R    # regression_tool_app() + run_regression_tool()
├── inst/
│   ├── geo/*.geojson.gz         # built-in choropleth boundaries (US states / counties, world countries); public domain, built by dev/build_boundaries.R
...
└── tests/
    ...
    └── testthat/                # setup.R (no longer sources R/); test-<area>.R per helper
        ...
        ├── test-state.R    test-lmer.R   test-map.R  test-glmm.R
        ├── test-examples.R                           # the example registry (0.11.0)
        └── test-modules.R  test-launchers.R          # the app layer (0.10.0)
```
...
R + `shiny`, `bslib` (layout: `page_navbar`, `layout_sidebar`, `card`, `navset_card_tab` — matches the existing app), `DT`, `tidyr`, `dplyr`, `tidyselect`, `ggplot2`, `plotly` (interactive chart + regression-diagnostic previews via `ggplotly()`), `colourpicker`, `binom` (exact CIs), `base64enc` / `tibble`, `hexbin` (the hexbin chart, Suggests-gated), `writexl` / `readxl` (import/export), `officer` (the editable Word report — pandoc-free), `here`, and `testthat` for tests. The **Mixed Model Review** app adds the mixed-model stack: `lmerTest` (loads `lme4`), `emmeans`, `multcomp` + `multcompView` (the cld letter engine), and `lattice` (the random-effects caterpillar) in Imports; `pbkrtest` (Kenward-Roger df), `performance` / `MuMIn` (R²/ICC) are optional (Suggests, graceful `requireNamespace` fallback); `influence.ME` is Suggests-listed only because the emitted code offers it — `lmer_cook()` computes leave-one-group-out distances directly and never calls it. The **GLMM Review** app adds `glmmTMB` (compiled, loads TMB) + `DHARMa` (Imports) with `car` (Type III Wald ANOVA — Suggests, `requireNamespace`-gated, the tab degrades to an instructive note without it). The **Map Tool** adds `leaflet` + `htmlwidgets` + `htmltools` + `jsonlite` (Imports; all leaflet calls `pkg::`-qualified, no `@import`; jsonlite parses uploaded choropleth GeoJSON) with `leaflet.extras` (the density-heatmap layer, Suggests, requireNamespace-gated) and `webshot2` / `chromote` (PNG snapshots; needs a system Chrome — falls back to Edge via `CHROMOTE_CHROME`, retries in `CHROMOTE_HEADLESS=new` mode) in Suggests; the HTML download needs nothing external (`inline_html_deps` self-contains it, no pandoc). Add packages as needed and note them in the build log.
...
- **One theme, one source of truth.** `R/components.R` is the canonical look: `uf_theme()` (flatly + UF-blue navbar), the logo title, `info_tip()`, `label_or()`, `copy_js`, and `uf_busy()` (app-wide busy spinners, `header = uf_busy()` in every launcher; needs shiny >= 1.11.0). UF colors live there as `UF_BLUE`/`UF_ORANGE`/`UF_COLORS`. **Blue is `#003087`** (matches the original app; the early plan floated `#0021A5` — change the one constant in `components.R` if the brand guide says otherwise). Every app pulls its theme from here.
...
- **`shiny::testServer` works here** (it segfaulted on the Windows box, which is why module coverage arrived late — see test-modules.R / test-launchers.R, added in 0.10.0). Read a module's return with `session$returned()`.
