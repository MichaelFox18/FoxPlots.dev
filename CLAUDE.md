...
- **A testthat suite** (`tests/testthat/`, ~1,600 expectations) run via `R CMD check` / `testthat::test_local()` (Word-report tests `skip_if_not_installed("officer")`; the mixed-model engine tests `skip_if_not_installed("lmerTest"/"emmeans")`). Since 0.10.0 the app layer is covered too: **`test-modules.R`** (one contract check per module) and **`test-launchers.R`** (each app builds, serves a real page, and has its whole server run via `testServer` on the app object, plus a structural check that every `<mod>Server("id")` has a matching `<mod>UI("id")` — a mistyped id otherwise runs in a namespace with no UI and the tab is silently blank). testServer **works on the macOS dev box** (the segfault was Windows-only); read a module's return with `session$returned()` — but modules that return a LIST (`mod_lmer`, `mod_glmm`) expose it as `session$returned` directly, not callable.
...
│   ├── helpers_plot.R           # build_full_plot (11 chart types), chart hints, palettes, code gen, grid export; caps BAR_MAX/PIE_MAX/GROUP_MAX
