...
- **A testthat suite** (`tests/testthat/`, ~940 expectations) run via `R CMD check` / `testthat::test_local()` (Word-report tests `skip_if_not_installed("officer")`; the mixed-model engine tests `skip_if_not_installed("lmerTest"/"emmeans")`). Modules verified with `shiny::testServer` smoke checks — testServer **works on the macOS dev box** (the segfault was Windows-only); read a module's return with `session$returned()`.
...
│   ├── helpers_map.R            # map engine: coord detect/clean, build_leaflet_map, generate_map_code, palettes, built-in boundaries (MAP_BUILTIN_BOUNDARIES + builtin_boundary_*), make_map_example_data, HTML/PNG export
